...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String loginId) throws UsernameNotFoundException {
        ...
        UserVO customUser = userSecurityService.getUser(loginId);
        if(customUser == null)
            throw new UsernameNotFoundException("사용자가 입력한 아이디에 해당하는 사용자를 찾을 수 없습니다.");

        CustomUserDetails customUserDetails = new CustomUserDetails();
        customUserDetails.setUsername(customUser.getUserEmail());
        customUserDetails.setPassword(customUser.getUserPassword());

        List<UserRoleVO> customRoles = userSecurityService.getUserRoles(loginId);
